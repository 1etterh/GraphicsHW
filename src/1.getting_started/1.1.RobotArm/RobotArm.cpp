...
const char* ourObjectPath = "./Textured_mesh_2.obj";
//const char* ourObjectPath = "./teapot.obj";
// translate it so it's at the center of the scene
// it's a bit too big for our scene, so scale it down
glm::mat4 objectXform = glm::scale(glm::translate(glm::mat4(1.0f), glm::vec3(0.5f, 0.3f, 0.0f)), glm::vec3(0.02f, 0.02f, 0.02f));
glm::mat4 diff = objectXform;
...
void myDisplay()
{
	...
	/*
	Drawing Base on the left of teapot
	*/
	glm::mat4 baseModel = glm::mat4(1.0f);
	baseModel = glm::translate(baseModel, glm::vec3(BaseTransX, 0.0f, BaseTransZ));
	baseModel = glm::rotate(baseModel, glm::radians(BaseSpin), glm::vec3(0.0f, 1.0f, 0.0f));
	DrawBase(baseModel);
	...
	/*
	Drawing Arm Segment1 at the top of Base, using relative position

	*/
	glm::mat4 armSegmentModel_1 = glm::translate(baseModel, glm::vec3(0.0f, 0.4f, 0.0f));
	armSegmentModel_1 = glm::rotate(armSegmentModel_1, glm::radians(ShoulderAng), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawArmSegment(armSegmentModel_1);

	/*
	Draw Arm Segment2 using relative position of armSegmentModel_1

	*/

	glm::mat4 armSegmentModel_2 = glm::translate(armSegmentModel_1, glm::vec3(0.0f, 0.5f, 0.0f));
	armSegmentModel_2 = glm::rotate(armSegmentModel_2, glm::radians(ElbowAng), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawArmSegment(armSegmentModel_2);


	/*
	Draw Wrist Using relative location of armSegMent_2
	*/

	glm::mat4 wristModel = glm::translate(armSegmentModel_2, glm::vec3(0.0f, 0.5f, 0.0f));
	wristModel = glm::rotate(wristModel, glm::radians(WristAng), glm::vec3(0.0f, 0.0f, 1.0f));
	DrawWrist(wristModel);




	/*
	DrawFingerBases Using relative location of wrist
	*/

	glm::mat4 wristJoint = glm::translate(wristModel, glm::vec3(0.0f, 0.2f, 0.0f));
	glm::mat4 fingerModel_1 = glm::rotate(wristJoint, glm::radians(FingerAng1), glm::vec3(0.0f, 0.0f, 1.0f));
	glm::mat4 fingerModel_2 = glm::rotate(wristJoint, glm::radians(FingerAng1 * -1.0f), glm::vec3(0.0f, 0.0f, 1.0f));

	DrawFingerBase(fingerModel_1);
	DrawFingerBase(fingerModel_2);


	/*
	DrawFingerTips Using relative location of FingerBases
	*/

	glm::mat4 fingerTip_1 = glm::translate(fingerModel_1, glm::vec3(0.0f, 0.35f, 0.0f));
	fingerTip_1 = glm::rotate(fingerTip_1, glm::radians(FingerAng2), glm::vec3(0.0f, 0.0f, 1.0f));
	glm::mat4 fingerTip_2 = glm::translate(fingerModel_2, glm::vec3(0.0f, 0.35f, 0.0f));
	fingerTip_2 = glm::rotate(fingerTip_2, glm::radians(FingerAng2 * -1.0f), glm::vec3(0.0f, 0.0f, 1.0f));

	DrawFingerTip(fingerTip_1);
	DrawFingerTip(fingerTip_2);
	...
	*/
	...
	if (spaceBarPressed == 0) {
		glm::mat4 InJoint = glm::inverse(wristJoint);
		diff = InJoint * objectXform;
		
	}
	else if (spaceBarPressed) {
		objectXform = wristJoint * diff;
	}
...
void processInput(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	if (key >= GLFW_KEY_1 && key <= GLFW_KEY_5 && action == GLFW_PRESS)
	{
		...
		/*printf("%d", RobotControl);*/

	}
	else if (key == GLFW_KEY_ESCAPE && action == GLFW_PRESS)
		glfwSetWindowShouldClose(window, true);
	else if (key == GLFW_KEY_SPACE && action == GLFW_PRESS && spaceBarPressed == 0)
	{
		spaceBarPressed = 1;
		printf("%d", spaceBarPressed);
	}
...
void mouse_callback(GLFWwindow* window, double xpos, double ypos)
{
	...
	/*xoffset, yoffset 계산방식을 LearnOpenGL의 camera_class.cpp의 방식으로 변경 */
	float xoffset_Mouse = xpos - lastX;
	float yoffset_Mouse = lastY - ypos;
	...
	{
		camera.ProcessMouseMovement(xoffset_Mouse, yoffset_Mouse);
	}
